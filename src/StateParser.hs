...
happyExpList = Happy_Data_Array.listArray (0,22) ([1024,512,64,0,128,128,128,5,1,0,8,0,1024,0,1280,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_state_p","Stat","VarsOrEmpty","Vars","Int","int","var","'-'","'['","']'","','","'->'","%eof"]
        bit_start = st * 15
        bit_end = (st + 1) * 15
...
action_15 (10) = happyShift action_13
...
happyReduction_4 (HappyAbsSyn7  happy_var_3)
	_
	(HappyTerminal (TokenStateVar happy_var_1))
	 =  HappyAbsSyn6
		 (state [(happy_var_1,happy_var_3)]
	)
happyReduction_4 _ _ _  = notHappyAtAll 

happyReduce_5 = happyReduce 5 6 happyReduction_5
happyReduction_5 ((HappyAbsSyn7  happy_var_5) `HappyStk`
	_ `HappyStk`
	(HappyTerminal (TokenStateVar happy_var_3)) `HappyStk`
	_ `HappyStk`
	(HappyAbsSyn6  happy_var_1) `HappyStk`
	happyRest)
	 = HappyAbsSyn6
		 (update_entry (happy_var_3,happy_var_5) happy_var_1
	) `HappyStk` happyRest

happyReduce_6 = happySpecReduce_1  7 happyReduction_6
happyReduction_6 (HappyTerminal (TokenStateInt happy_var_1))
	 =  HappyAbsSyn7
		 (happy_var_1
	)
...
happyReduction_7 (HappyTerminal (TokenStateInt happy_var_2))
	_
	 =  HappyAbsSyn7
		 (-happy_var_2
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	TokenStateInt happy_dollar_dollar -> cont 8;
	TokenStateVar happy_dollar_dollar -> cont 9;
	TokenStateNegate -> cont 10;
	TokenStateOB -> cont 11;
	TokenStateCB -> cont 12;
	TokenStateComma -> cont 13;
	TokenStateArrow -> cont 14;
	_ -> happyError' ((tk:tks), [])
	}
...
happyError' :: () => ([(TokenState)], [String]) -> HappyIdentity a
happyError' = HappyIdentity . (\(tokens, _) -> parseErrorState tokens)
state_p tks = happyRunIdentity happySomeParser where
